...
#
# $Id: 10_MQTT_GENERIC_BRIDGE.pm 23653+AnalyzePerlCommand 2021-02-09 Beta-User $
#
###############################################################################
...
use warnings;
use AttrTemplate;
use Carp qw(croak carp); #Beta-User: croak not used yet

##no critic qw(constant Package)

use GPUtils qw(:all);    #Beta-User: really all?
...
#    EvalSpecials
#    perlSyntaxCheck might be further candidates
...
sub CreateSingleDeviceTableAttrDefaults { #($$$$) {
  ...
  #  return;
  #}
  #return; #not needed if earlier if !defined statement is used
}
...
sub computeDefaults { #($$$$$) {
  ...
  if (defined($globalMap) and defined($globalMap->{':defaults'})) {
    for my $param (keys %{$globalMap->{':defaults'}} ) {
      if(startsWith($param,$modifier)) {
        ...
        $val = _evalValue2($hash,$val,$infoMap);
  ...
  for my $param (keys %{$devCombined} ) {
    ...
    $devCombined->{$param} = _evalValue2($hash,$val,{%$defaultCombined, %$infoMap});
  }
...
sub CreateSingleDeviceTableAttrSubscribe { #($$$$) {
  ...
  my $attrVal = shift; #seems not to be mandatory...
          ...
          while (@nameParts) {
            ...
            if(($ident eq 'topic') or 
             #($ident eq 'sttopic') or
              ...
              my $topic;
              $topic = _evalValue2($hash,$val,{'device'=>$dev,'reading'=>'#reading','name'=>'#name',%$combined}) if defined $val;
              if(!defined($topic)) {
...
sub checkPublishDeviceReadingsUpdates {
  ...
  unless (defined($devDataRecord)) {
    ...
    #Beta-User - evtl. sowas: return "" if !(%{$globalPublishMap});
  }
...
sub defineGlobalDevExclude { #($;$) {
  ...
  #$valueName.= ' '.DEFAULT_GLOBAL_DEV_EXCLUDES if defined $valueName; #Beta-User: Logikfehler? Wenn, dann müßte man die beiden vorangehenden Zeilen umdrehen, oder?
  ...
  for my $val (@$unnamed) {
    ...
    next if($dev eq '');
    $reading = '*' if !defined $reading || $reading eq '';
    #$reading = '*' if $reading eq '';
    ...
    for my $pubRec (@$pubRecList) {
      if(defined($pubRec)) { 
        ...
        if(defined $expression) {
          ...
          my $ret;
          if ($useOldEval) {
            $ret = _evalValue2($hash,$expression,{'topic'=>$topic,'device'=>$devn,'reading'=>$reading,'name'=>$name,'time'=>TimeNow(),%$defMap},1);
            $ret = eval($ret);
          } else { 
...
sub onmessage {
  ...
  my $updated = 0; # Beta-User: kann m.E. auch weg; am Ende wissen wir ja, ob das Array einen Inhalt hat...
  my @updatedList;
  for my $deviceKey (keys %{$fMap}) {
        ...
        if(defined $expression) {
          ...
          #  $message = undef;
            ...
            $message = $ret // undef; #Beta-User: sicherheitshalber...
          }
          #Log3($hash->{NAME},1,"MQTT_GENERIC_BRIDGE:DEBUG:> [$hash->{NAME}] eval done: ".Dumper($ret));
          if ($@) {
